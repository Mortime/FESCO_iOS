...
{
    [self beginRefreshingWithAniamtion:YES];
}
...
-(void)beginRefreshingWithAniamtion:(BOOL)aniamtin
{
    if (!isRefresh) {
        ...
        if (aniamtin) {
            [UIView animateWithDuration:0.3 animations:^{
                _scrollView.contentInset = UIEdgeInsetsMake(headerHeight*1.5, _scrollView.contentInset.left, 0, _scrollView.contentInset.right);
                _scrollView.contentOffset = CGPointMake(_scrollView.contentOffset.x, -headerHeight * 1.5);
            }];
        }else{
            [CATransaction begin];
